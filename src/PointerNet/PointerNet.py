...
class Encoder(nn.Module):
    ...
    def __init__(self, embedding_dim,
        ...
        :param bool bidir: if Bidirectional
        """
    ...
    def init_hidden(self, embedded_inputs):
        ...
        if self.bidir:
            h0 = self.h0.unsqueeze(0).unsqueeze(0).repeat(self.n_layers*2,
                                                          batch_size,
                                                          self.hidden_dim)
            c0 = self.h0.unsqueeze(0).unsqueeze(0).repeat(self.n_layers*2,
                                                          batch_size,
                                                          self.hidden_dim)
        else:
...
class PoemEmbedding(nn.Embedding):
    def __init__(self, vocab_size, embedding_dim, init_weight=None):
        """
...
class Decoder(nn.Module):
    ...
    def forward(self, embedded_inputs,
        ...
        :param Tensor hidden: First decoder's hidden states, [1, bz, hidden_size]
        ...
        # Generating arrange(input_length), broadcasted across batch_size
        # -> [bz, seq_len]
        ...
        for _ in range(5):
            ...
            # Masking selected inputs([bz, seq_len])
            ...
            outputs.append(outs.unsqueeze(0))    # [1, bz, hidden_size]
...
class PointerNet(nn.Module):
    ...
    def __init__(self, embedding_dim,
        ...
        :param float dropout: Float between 0-1
        :param bool bidir: if set encoder Bidirectional
        """
    ...
    def forward(self, inputs):
        ...
        :param Tensor inputs: Input sequence, [bz, len, 2]
        ...
        # -> [1, bz, hidden_size]
        if self.bidir:
            decoder_hidden0 = (torch.cat((encoder_hidden[0][-1], encoder_hidden0[0][-2]), dim=-1),
                               torch.cat((encoder_hidden[1][-1], encoder_hidden[1][-2]), dim=-1))
        else:
